...
namespace SchoolManager.Areas.Medical.Controllers
{
    ...
    public class PsychologyController : Controller
    {
        ...
        public async Task<IActionResult> DescargarPDF(string filtro, DateTime? fechaInicio, DateTime? fechaFin)
        {
            ...
            document.Add(new Paragraph("DEPARTAMENTO DE PSICOLOGÍA")
                .SetFont(fontBold).SetFontSize(16)
                .SetTextAlignment(TextAlignment.CENTER));

            document.Add(new Paragraph("Reporte de Atenciones Psicológicas")
            ...
            document.Add(new Paragraph($"Total de registros: {datos.Count}")
                .SetFont(fontBold).SetFontSize(11)
                .SetMarginBottom(5));

            var porAsistencia = datos.GroupBy(x => x.Asistencia)
                .Select(g => $"{g.Key}: {g.Count()}")
                .ToList();

            document.Add(new Paragraph("Desglose por asistencia: " + string.Join("   |   ", porAsistencia))
                .SetFont(fontNormal).SetFontSize(10)
                .SetMarginBottom(15));

            var tabla = new Table(UnitValue.CreatePercentArray(new float[] { 12, 18, 30, 20, 20 }))
                .UseAllAvailableWidth();

            string[] headers = { "Folio", "Matrícula", "Alumno", "Asistencia", "Fecha" };
        ...
        public async Task<IActionResult> Create(medical_pychology model)
        {
            ...
            var ultimoFol = await _context.MedicalPsychology
