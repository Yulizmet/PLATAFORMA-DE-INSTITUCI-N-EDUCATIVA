...
namespace SchoolManager.Areas.Medical.Controllers
{
    ...
    public class LogbookController : Controller
    {
        ...
        public async Task<IActionResult> DescargarPDF(string filtro, DateTime? fechaInicio, DateTime? fechaFin)
        {
            ...
            // TÍTULO
            document.Add(new Paragraph("DEPARTAMENTO MÉDICO")
                .SetFont(fontBold).SetFontSize(16)
                .SetTextAlignment(TextAlignment.CENTER));

            document.Add(new Paragraph("Reporte de Atenciones Clínicas")
            ...
            // RESUMEN
            document.Add(new Paragraph($"Total de atenciones: {datos.Count}")
                .SetFont(fontBold).SetFontSize(11)
                .SetMarginBottom(5));

            var porEstado = datos.GroupBy(x => x.Estado)
                .Select(g => $"{g.Key}: {g.Count()}")
                .ToList();

            document.Add(new Paragraph("Desglose por estado: " + string.Join("   |   ", porEstado))
                .SetFont(fontNormal).SetFontSize(10)
                .SetMarginBottom(15));

            // TABLA
            var tabla = new Table(UnitValue.CreatePercentArray(new float[] { 10, 15, 25, 25, 15, 15 }))
                .UseAllAvailableWidth();

            // ENCABEZADOS
            string[] headers = { "Folio", "Matrícula", "Alumno", "Motivo", "Estado", "Fecha" };
            foreach (var h in headers)
            {
                tabla.AddHeaderCell(new Cell()
                    .Add(new Paragraph(h).SetFont(fontBold).SetFontSize(9).SetFontColor(colorHeaderText))
