...
// ===============================
// CP MX -> Colonias (y Estado/Municipio) - COPOMEX (robusto)
// ===============================
(() => {
  "use strict";

  const TOKEN = "02fcf8dc-cceb-42f9-923c-a927ef47d5a8"; // tu token
  ...
  const API = "https://api.copomex.com/query/";
 ...
 function normalizeCopomexPayload(data) {
  // Caso 1: Copomex devuelve ARREGLO de objetos (lo que tú viste: [{...}])
  if (Array.isArray(data) && data.length && typeof data[0] === "object") {
    const first = data[0];

    // Colonias pueden venir como "asentamiento" (muy común) o "colonia"
    const colonias = uniqSort(
      data
        .map((x) => x.asentamiento || x.colonia || x.colonias || x.nombre)
        .filter(Boolean)
    );

    const estado = first.estado || first.estado_nombre || first.state || "";
    const municipio = first.municipio || first.municipio_nombre || first.city || "";

    return { colonias, estado, municipio };
  }

  // Caso 2: devuelve objeto con {respuesta:{...}} o directamente {...}
  const root = data;
  const r = root?.respuesta ?? root;

  // Colonias como array/string en un campo directo
  const coloniasRaw = r?.colonia ?? r?.colonias ?? r?.asentamiento ?? r?.asentamientos;
  const colonias = Array.isArray(coloniasRaw)
    ? coloniasRaw
    : coloniasRaw
    ? [coloniasRaw]
    : [];

  const estado = r?.estado ?? r?.estado_nombre ?? "";
  const municipio = r?.municipio ?? r?.municipio_nombre ?? "";

  return { colonias: uniqSort(colonias), estado, municipio };
}


  async function fetchInfoByCP(cp) {
    const url = `${API}info_cp/${encodeURIComponent(cp)}?token=${encodeURIComponent(TOKEN)}`;
    const data = await getJson(url);
    return normalizeCopomexPayload(data);
  }

  async function fetchColoniasAlt(cp) {
    const url = `${API}get_colonia_por_cp/${encodeURIComponent(cp)}?token=${encodeURIComponent(TOKEN)}`;
    const data = await getJson(url);

    // Este endpoint muchas veces trae arreglo directo o {respuesta:[...]}
    const root = Array.isArray(data) ? data : (data?.respuesta ?? []);
  ...
  async function loadByCP(cp) {
    ...
    resetSelect(estadoSelect, "Sin selección");
    resetSelect(municipioSelect, "Sin selección");

    try {
      let { colonias, estado, municipio } = await fetchInfoByCP(cp);

      // fallback si info_cp no trae colonias
      if (!colonias.length) {
        colonias = await fetchColoniasAlt(cp);
      }
      ...
      fillSelect(coloniaSelect, colonias);

      if (estadoSelect && estado) fillSelect(estadoSelect, [estado]);
      if (municipioSelect && municipio) fillSelect(municipioSelect, [municipio]);
    } catch (err) {
      if (cp !== lastCpRequested) return;
      resetSelect(coloniaSelect, "Sin selección");
      showError(true);
