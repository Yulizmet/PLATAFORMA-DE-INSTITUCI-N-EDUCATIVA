(function () {
    ...
    window.convertirPdf = async function convertirPdf() {
        try {
            ...
            const folio = _upper(_safe(_txt("cpFolio")));
            const nombre = _upper(_safe(_txt("cpNombre")));
            const carrera = _upper(_safe(_txt("cpEspecialidad")));

            const aspNombreCompleto = nombre;
            const aspGenero = _upper(_safe(_txt("pdfGenero")));
            const fechaNacimientoRaw = _txt("pdfFechaNacimiento");
            const aspFechaNacFmt = _safe(formatFechaMX(fechaNacimientoRaw));
            const aspEdad = _safe(calcEdad(fechaNacimientoRaw));
            const aspCurp = _upper(_safe(_txt("pdfCurp")));
            const aspTipoSangre = _upper(_safe(_txt("pdfTipoSangre")));

            const aspProcedencia = _upper(_safe(_txt("pdfSecundaria")));
            const aspPromedio = _safe(_txt("pdfPromedio"));

            const domEstado = _upper(_safe(_txt("pdfEstado")));
            const domMunicipio = _upper(_safe(_txt("pdfMunicipio")));

            const calle = _txt("pdfCalle");
            const numExt = _txt("pdfNumeroExterior");
            const numInt = _txt("pdfNumeroInterior");
            const colonia = _txt("pdfColonia");
            const cp = _txt("pdfCodigoPostal");

            const domLinea = _upper(_safe(
                [
            ...
            const fechaFinSec = _txt("pdfFechaFinSecundaria");
            const fechaFinDate = parseDateLoose(fechaFinSec);
            const anioConcluyo = fechaFinDate ? String(fechaFinDate.getFullYear()) : "—";

            const generacion = _safe(_txt("pdfGeneracion"));
